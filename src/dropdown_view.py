...
class DropDownView(disnake.ui.View):
    ...
    async def update_content(self, button: disnake.ui.Button, interaction: disnake.MessageInteraction):
        ...
        search_page = search("[0-9]", title).start()
        # Extract the index by converting the digit to an integer and subtracting 1
        self._embed_dropdown.index = int(title[search_page]) - 1
